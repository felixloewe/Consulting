...
Top_Export <- function(Year){
  top_export <- aggregate(MADdata[MADdata$Year == Year,]$Value,
                          list(Land = MADdata[MADdata$Year == Year,]$Reporter_Name, ccode = MADdata[MADdata$Year == Year,]$Reporter_Code ),
                          sum)
...
Top_Import <- function(Year){
  top_import <- aggregate(MADdata[MADdata$Year == Year,]$Value,
                          list(Land = MADdata[MADdata$Year == Year,]$Partner_Name, ccode = MADdata[MADdata$Year == Year,]$Partner_Code),
                          sum)
